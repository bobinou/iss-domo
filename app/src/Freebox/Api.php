...
class Api
{
	...
	/**
	* Constructeur
	* @param array configuration if no configuraiton is given, load the base from app/config/freebox.php file
	*/
	public function __construct($config = null)
	{
		if (!$config) {
			$config = \Config::get('freebox');
		}

		if (!$config) {
			throw new \Exception('Configuration file for freebox invalid or not found');
		}

		// On assigne les paramètres aux variables d'instance.
		$this->IP = $config['freebox_server_url'];
	...
	*/
	public function _RequestAuth($force_refresh = 0)
	{
		...
		$TokenFile = storage_path('freebox/token');
		$appParams = array (
				"app_id" => $this->APP_ID,
		...
		if ((!file_exists($TokenFile)) || ($force_refresh == 1))
		{
			...
			if ($result['success'] == true)
			{
				if ((is_writable($TokenFile)) || (!file_exists($TokenFile)))
				{
					...
					{
						throw new \Exception("Impossible d'ouvrir le token $TokenFile");
					}
	...
	*/
	public function _NewSession()
	{
		...
		if (strcmp($response['result']['status'],'granted') != 0)
		{
			...
			{
				sleep(1);
				Log::debug('Attente d\'autorisation pour l\'application. Validez l\'accès depuis l\'afficheur de la Freebox Server.');
				$response = $this->_GetAuthStatus();
			}
			if (strcmp($response['result']['status'],'granted') != 0)
			{
				throw new \Exception("Request access ".$response['result']['status']." !");
			}
		}
	...
	*/
	...
	public function GetListClasses()
	{
		...
		foreach (glob(dirname(__FILE__) . '/API/*.php') as $file) {
			$classes[] = '\\'.__NAMESPACE__.'\\API\\'.pathinfo($file, PATHINFO_FILENAME);
		}
		return $classes;
	}
	...
	public function config_to_XML()
	{
		...
		{
			$classnameNode = $doc->createCDATASection($class_name);
			$Classe_Elt = $doc->createElement('class');
			$Classe_Elt->appendChild($classnameNode);
	...
	public function config()
	{
		...
		foreach ($array_classes as $class_name)
		{
			$classdata = array('name' => $class_name, 'methods' => array());
			$class_methods = get_class_methods($class_name);
			$class = new $class_name($this);
			foreach ($class_methods as $method_name)
			{
				if (substr($method_name,0,3) === "Get")
				{
					...
					{
						$datas = call_user_func(array($class, $method_name));
						//$classdata['methods'][$method_name] = $datas['result'];
						$classdata = $datas['result'];
					}
				}
			}
			//$output['freeboxOS']['classes'][] = $classdata;
			$output[] = $classdata;
		}
		return $output;
	}
